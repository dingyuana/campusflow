...
1. 安装 Neo4j 依赖（使用国内镜像）：
   ```bash
   uv pip install neo4j langchain-openai --index-url https://pypi.tuna.tsinghua.edu.cn/simple
   ```
   （OpenAI 用于 Text-to-Cypher，可替换为开源模型）
...
3. Supabase 表设计与 RLS（行级安全）策略介绍（20min）；
...
2. 登录 Supabase Dashboard → Table Editor，验证表结构是否正确。
...
1. 提交代码：git add . → git commit -m "feat: 完成Supabase业务表设计与FastAPI接口开发"；
...
3. Checkpointer 核心组件：PostgresSaver（存储状态到 Supabase）、Thread ID（会话唯一标识）（30min）；
...
1. 安装 LangGraph 依赖（使用国内镜像）：
   ```bash
   uv pip install langgraph langgraph-checkpoint-postgres --index-url https://pypi.tuna.tsinghua.edu.cn/simple
   ```
2. 确保 Supabase 连接正常（.env中 SUPABASE_DB_URL 配置正确）。
...
2. 登录 Supabase Dashboard → Table Editor，查看 langgraph_checkpoints 表，确认状态数据已存储。
